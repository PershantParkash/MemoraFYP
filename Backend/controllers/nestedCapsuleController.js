...
export const createNestedCapsule = async (req, res) => {
  try {
    ...
    // Handle file uploads - Make this consistent with createTimeCapsule
    let media = null;
    let finalDescription = description;
    ...
    if (req.files && req.files.length > 0) {
      ...
      const videoFile = req.files.find(file => file.mimetype.startsWith('video/')); // Added video support

      console.log('Found files:', {
        imageFile: imageFile ? { name: imageFile.originalname, mimetype: imageFile.mimetype } : null,
        audioFile: audioFile ? { name: audioFile.originalname, mimetype: audioFile.mimetype } : null,
        videoFile: videoFile ? { name: videoFile.originalname, mimetype: videoFile.mimetype } : null, // Added video logging
      });

      // Use the same priority logic as createTimeCapsule
      if (imageFile) {
        media = imageFile.filename;
        console.log('Image file saved as:', media);
      } else if (audioFile) {
    ...
    console.log("Parent Capsule ID type:", typeof parentCapsuleId, parentCapsuleId);
    console.log('Creating nested capsule with:', {
      Title: title,
      Description: finalDescription,
      CapsuleType: capsuleType,
